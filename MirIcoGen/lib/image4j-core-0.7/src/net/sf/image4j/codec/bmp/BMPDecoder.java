...
public class BMPDecoder {
  ...
  public BMPDecoder(java.io.InputStream in) throws IOException {
    ...
    int pos = 40 + 14;
    if (colorTable != null) {
      pos += colorTable.length * 4;
    }
    if (dataOffset > pos) {
      lis.skipBytes(dataOffset - pos);
    }
  ...
  public static ColorEntry[] readColorTable(InfoHeader infoHeader, net.sf.image4j.io.LittleEndianInputStream lis) throws IOException {  
     // Sometimes you have a bitmap where iNumColors is 2 but only 1 is actually used and the image data directly follows it.
     int numColors = infoHeader.iColorsUsed;
     if (numColors == 0) {
       numColors = infoHeader.iNumColors;
     }
 
     ColorEntry[] colorTable = new ColorEntry[numColors];
